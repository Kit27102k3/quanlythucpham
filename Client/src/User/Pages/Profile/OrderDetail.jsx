...
import { geocodeAddressDebounced, SHOP_LOCATION } from "./OrderDetail/MapUtils";
...
export default function OrderDetail() {
  ...
  const createStraightLine = useCallback(
    (map, customerCoords, distanceStraightLine) => {
  ...
  const addMapMarkers = useCallback(
    ...
    // Tạo marker cho vị trí cửa hàng - Sửa lại để hiển thị rõ hơn
      ...
      shopEl.style.width = "40px"; // Tăng kích thước
      shopEl.style.height = "40px"; // Tăng kích thước
    ...
    const distanceStraightLine = calculateDistance(
      ...
      console.log(
        `Distance between shop and customer: ${distanceStraightLine.toFixed(
          2
        )}km`
      );
    ...
    const directionsRequest = `https://api.mapbox.com/directions/v5/mapbox/driving/${SHOP_LOCATION.lng},${SHOP_LOCATION.lat};${customerCoords.lng},${customerCoords.lat}?steps=true&geometries=geojson&overview=full&access_token=${MAPBOX_ACCESS_TOKEN}`;
    
      console.log(
        "Requesting route from Mapbox Directions API for driving route"
      );
  ...
  const initMap = useCallback(() => {
    try {
      console.log(
        ...
        mapboxgl.accessToken ? "Token exists" : "Missing token"
      );
      console.log(
        "Window orderTrackingMap:",
        window.orderTrackingMap ? "exists" : "doesn't exist"
      );
      ...
      console.log(
        "Mapbox GL initialized with token:",
        mapboxgl.accessToken.substring(0, 5) + "..."
      );
  ...
  const performGeocoding = useCallback(
    ...
    if (window._geocoding_requested === address) {
      console.log("Đã yêu cầu geocoding cho địa chỉ này trước đó:", address);
          return;
        }
        
    console.log("Thực hiện geocoding cho địa chỉ:", address);
    console.log("Tỉnh/thành phố được trích xuất:", province || "Không có");
    
    // Đánh dấu đã yêu cầu geocoding để tránh gọi lại
    window._geocoding_requested = address;
    setGeocodingRequested(true);
    
    // Sử dụng hàm geocoding có debounce và cache
    geocodeAddressDebounced(address, (result) => {
        console.log("Kết quả geocoding:", result);
  ...
  useEffect(() => {
    if (order && !customerLocation?.initialized && !geocodingRequested) {
      ...
      console.log(
        "Địa chỉ có chứa tỉnh ĐBSCL:",
        isMekongDelta ? "Có" : "Không"
      );
      ...
      if (
        ...
        if (distance > 1500) {
          console.log(
            `Tọa độ từ database cách shop ${distance.toFixed(
              2
            )}km, có thể không chính xác. Thực hiện geocoding lại.`
          );
        ...
        setTimeout(() => {
          try {
              console.log(
                "Khởi tạo bản đồ với tọa độ từ database:",
        ...
        try {
          ...
          if (savedLocations[addressKey]) {
            ...
            if (daysDiff <= 7) {
              ...
              if (distance <= 200) {
                console.log("Sử dụng tọa độ đã lưu cho địa chỉ:", fullAddress);
          ...
          setTimeout(() => {
            try {
              ...
              setMapError(true);
                ...
                console.log(
                  "Tọa độ đã lưu không hợp lý (cách shop",
                  distance.toFixed(2),
                  "km), thực hiện geocoding lại"
                );
        ...
        try {
          ...
          if (geocodingCache[addressKey]) {
            ...
            if (distance > 1500) {
              console.log(
                `Kết quả cache cách shop ${distance.toFixed(
                  2
                )}km, có thể không chính xác. Xóa khỏi cache.`
              );
              ...
              console.log(
                "Sử dụng kết quả geocoding từ cache cho địa chỉ:",
                fullAddress
              );
              ...
              // Khởi tạo bản đồ với dữ liệu từ cache
                setTimeout(() => {
                  try {
                  console.log(
                    "Khởi tạo bản đồ với tọa độ từ cache:",
                    customerLocationData
                  );
                  initMap(customerLocationData);
                  } catch (err) {
        ...
        console.log(
          "Không có tọa độ từ database hoặc cache, thực hiện geocoding cho địa chỉ:",
          fullAddress
        );
  ...
  useEffect(() => {
    ...
    if (
      ...
      try {
        console.log(
          "Thực hiện geocoding địa chỉ khách hàng:",
          customerLocation.address
        );
      ...
      return;
        ...
        if (isMounted) {
          ...
          if (orderData.orderCode) {
            console.log(
              "Đơn hàng có mã vận đơn, tải thông tin tracking:",
              orderData.orderCode
            );
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "Khởi tạo vị trí mặc định cho bản đồ vì không có customerLocation"
      );
  ...
  useEffect(() => {
    if (mapError && customerLocation) {
      console.log(
        "El mapa interactivo ha fallado, utilizando mapa estático como alternativa"
      );
    ...
    const timer = setTimeout(() => {
      ...
      const attemptSetup = () => {
        ...
        if (!ensureMapContainer()) {
          console.log(
            `Attempt ${retryCount + 1} failed, trying again in ${
              500 * (retryCount + 1)
            }ms`
          );
          retryCount++;
          ...
          console.log(
            "Container ready and we have customer location, initializing map"
          );
          initMap(customerLocation);
        }
      };
  ...
  useEffect(() => {
    if (customerLocation && mapLoaded && !window.__map_initialized) {
      console.log(
        "Customer location available and map loaded, initializing map",
        customerLocation
      );
      setTimeout(() => {
        try {
  ...
  useEffect(() => {
    ...
    console.log("[Map] Khởi tạo map container một lần duy nhất khi mount");
    const ensureMapContainerExists = () => {
      ...
      if (mapSection) {
        ...
        if (!mapContainer) {
          console.log("Creating map container element (map-container)");
          mapContainer = document.createElement("div");
