...
function TopRated() {
  ...
  const bannerImages = [
    {
      ...
      alt: "Sản phẩm đánh giá cao nhất"
    },
    {
      src: "https://images.pexels.com/photos/4033637/pexels-photo-4033637.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
      alt: "Sản phẩm được yêu thích"
    }
  ];
  ...
  useEffect(() => {
    const fetchTopRatedProducts = async () => {
      try {
        ...
        try {
          const response = await axios.get(`${API_URLS.PRODUCTS}/top-rated?limit=4`);
  ...
  const handleTouchEnd = () => {
    ...
    if (isLeftSwipe && currentPage < getTotalPages(topRatedProducts) - 1) {
  ...
  // Return null if no top rated products found
  ...
  return (
    <div className="mt-10 mb-12">
      <div className="mb-6 text-center lg:text-left">
        ...
        <h1 className="text-[26px] font-medium mt-2 lg:text-[40px]">ĐƯỢC YÊU THÍCH NHẤT</h1>
        <p className="text-gray-500 text-[13px] lg:text-[16px]">
          Những sản phẩm được khách hàng đánh giá cao nhất
        </p>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          >
            {getPaginatedProducts(topRatedProducts, currentPage).map((product, index) => {
                ...
                >
                  <div className="relative">
                    ...
                    {productData.averageRating > 0 && (
                      <div className="absolute top-0 left-0 bg-yellow-500 text-white px-2 py-1 text-xs font-semibold flex items-center">
      ...
      {/* Pagination dots */}
      {topRatedProducts.length > itemsPerPage && (
        <div className="w-full flex flex-col items-center mt-4">
          <div className="flex justify-center items-center">
            <div className="flex space-x-3">
              {Array.from({ length: getTotalPages(topRatedProducts) }, (_, index) => (
                <button
                  key={index}
