...
const LocateControl = ({ setPosition, reverseGeocode }) => {
  ...
  return (
    <div
      ...
      style={{ zIndex: 1000, marginBottom: "60px" }}
    >
...
const CustomZoomControl = () => {
  ...
  return (
    <div
      ...
      style={{ zIndex: 999, marginBottom: "20px", marginRight: "10px" }}
    >
      <div className="leaflet-control leaflet-bar">
        <a
          href="#"
    ...
    try {
        ...
        try {
          const mapboxResponse = await fetch(
            `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
              query
            )}.json?country=vn&language=vi&access_token=${MAPBOX_ACCESS_TOKEN}&limit=5`
          );
          ...
          if (mapboxResponse.ok) {
            ...
            if (data && data.features && data.features.length > 0) {
              const formattedResults = data.features.map((feature) => {
                ...
                const secondaryPart = feature.place_name.replace(
                  mainPart + ", ",
                  ""
                );
      ...
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
          query
        )}&limit=5&countrycodes=vn&accept-language=vi&addressdetails=1`,
        {
      ...
      if (error.name === "AbortError") {
        ...
        setErrorMessage(
          "Không thể kết nối đến máy chủ tìm kiếm. Vui lòng kiểm tra kết nối mạng."
        );
      } else {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      if (MAPBOX_ACCESS_TOKEN) {
        try {
          const mapboxResponse = await fetch(
            `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
              searchValue
            )}.json?country=vn&language=vi&access_token=${MAPBOX_ACCESS_TOKEN}&limit=1`
          );
      ...
      const response = await fetch(
        `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
          searchValue
        )}&limit=1&countrycodes=vn&accept-language=vi&addressdetails=1`,
        {
          signal: controller.signal,
          headers: {
            "User-Agent": "FoodManagementApp/1.0",
    ...
    >
      ...
      {isMapReady && (
        <>
          ...
          <div
            className="leaflet-bottom leaflet-left"
            style={{ zIndex: 1000, marginBottom: "110px" }}
          >
            <div className="leaflet-control leaflet-bar">
              <button
                type="button"
