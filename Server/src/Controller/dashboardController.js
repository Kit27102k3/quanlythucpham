...
export const getDashboardStats = async (req, res) => {
    try {
        ...
        if (!productsByCategory || productsByCategory.length === 0) {
            ...
            // Ghi log để kiểm tra
            console.log("Dữ liệu phân loại sản phẩm:", JSON.stringify(finalProductsByCategory));
        }
        ...
        let finalTopProducts = topProducts.length > 0 ? topProducts : [
            { name: "Chưa có dữ liệu bán hàng", soldQuantity: 0 }
        ];

        // Ghi log để kiểm tra
        console.log("Top sản phẩm bán chạy:", JSON.stringify(finalTopProducts));
